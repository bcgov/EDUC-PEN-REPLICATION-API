...
/**
 * The type Add possible match orchestrator test.
 */
public class AddPossibleMatchOrchestratorTest extends BasePenReplicationAPITest {
  ...
  @Autowired
  private AddPossibleMatchOrchestrator orchestrator;
  @Autowired
  ...
  /**
   * Test add possible match given event and saga data should post event to services api.
   *
   * @throws IOException          the io exception
   ...
   */
  @Test
  public void testAddPossibleMatch_givenEventAndSagaData_shouldPostEventToServicesApi() throws IOException, InterruptedException, TimeoutException {
    ...
    assertThat(newEvent.getEventPayload()).contains("test-user").contains("MI");
  ...
  /**
   * Test create pen twins given event and saga data should post event to saga topic.
   *
   * @throws IOException          the io exception
   * @throws InterruptedException the interrupted exception
   * @throws TimeoutException     the timeout exception
   */
  @Test
  public void testCreatePenTwins_givenEventAndSagaData_shouldPostEventToSagaTopic() throws IOException, InterruptedException, TimeoutException {
    final var event = Event.builder()
  ...
  /**
   * Test create pen twins when students not found given event and saga data should post event to saga topic.
   *
   * @throws IOException          the io exception
   * @throws InterruptedException the interrupted exception
   * @throws TimeoutException     the timeout exception
   */
  @Test
  public void testCreatePenTwinsWhenStudentsNotFound_givenEventAndSagaData_shouldPostEventToSagaTopic() throws IOException, InterruptedException, TimeoutException {
    final var event = Event.builder()
      .eventType(ADD_POSSIBLE_MATCH)
    ...
    assertThat(sagaStates.size()).isEqualTo(1);
    assertThat(sagaStates.get(0).getSagaEventState()).isEqualTo(EventType.ADD_POSSIBLE_MATCH.toString());
    assertThat(sagaStates.get(0).getSagaEventOutcome()).isEqualTo(EventOutcome.STUDENTS_NOT_FOUND.toString());
  }
  ...
  public void testUpdateTwinsTransaction_givenEventAndSagaData_shouldPostEventToSagaTopic() throws IOException, InterruptedException, TimeoutException {
    ...
    final var event = Event.builder()
      ...
      .eventOutcome(EventOutcome.PEN_TWINS_CREATED)
      .sagaId(this.saga.getSagaId())
      .build();
    this.orchestrator.handleEvent(event);
    verify(this.messagePublisher, atLeastOnce()).dispatchMessage(eq(orchestrator.getTopicToSubscribe().getCode()), this.eventCaptor.capture());
    ...
    assertThat(newEvent.getEventPayload()).contains("{\"transactionID\":\"1234567890\",\"transactionType\":\"" + TransactionType.CREATE_TWINS.getCode() + "\",\"transactionStatus\":\"" + TransactionStatus.COMPLETE.getCode() + "\"");
    final var sagaFromDB = this.sagaService.findSagaById(this.saga.getSagaId());
    assertThat(sagaFromDB).isPresent();
    assertThat(sagaFromDB.get().getSagaState()).isEqualTo(UPDATE_PEN_TWIN_TRANSACTION.toString());
    final var sagaStates = this.sagaService.findAllSagaStates(this.saga);
    assertThat(sagaStates.size()).isEqualTo(1);
    assertThat(sagaStates.get(0).getSagaEventState()).isEqualTo(EventType.CREATE_PEN_TWINS.toString());
    assertThat(sagaStates.get(0).getSagaEventOutcome()).isEqualTo(EventOutcome.PEN_TWINS_CREATED.toString());
  }
  ...
  private PenTwinTransaction createMockPenTwinTransaction() {
    return PenTwinTransaction.builder()
      ...
      .penTwin1(this.pen1)
      .penTwin2(this.pen2)
      .transactionType(TransactionType.CREATE_TWINS.getCode())
