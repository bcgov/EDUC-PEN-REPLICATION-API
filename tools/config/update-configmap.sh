...
oc create -n "$PEN_NAMESPACE"-"$envValue" configmap "$APP_NAME"-config-map --from-literal=TZ=$TZVALUE --from-literal=COREG_API_URL="http://coreg-api-master.$COREG_NAMESPACE-$envValue.svc.cluster.local:8080/api/v1/course/information" --from-literal=STUDENT_API_URL="http://student-api-master.$COMMON_NAMESPACE-$envValue.svc.cluster.local:8080/api/v1/student" --from-literal=INSTITUTE_API_URL="http://institute-api-master.$COMMON_NAMESPACE-$envValue.svc.cluster.local:8080/api/v1/institute" --from-literal=TOKEN_URL="https://$SOAM_KC/auth/realms/$SOAM_KC_REALM_ID/protocol/openid-connect/token" --from-literal=JDBC_URL="$DB_JDBC_CONNECT_STRING" --from-literal=ORACLE_USERNAME="$DB_USER" --from-literal=ORACLE_PASSWORD="$DB_PWD" --from-literal=SPRING_SECURITY_LOG_LEVEL=INFO --from-literal=SPRING_WEB_LOG_LEVEL=INFO --from-literal=APP_LOG_LEVEL=INFO --from-literal=SPRING_BOOT_AUTOCONFIG_LOG_LEVEL=INFO --from-literal=SPRING_SHOW_REQUEST_DETAILS=false --from-literal=NATS_URL="$NATS_URL" --from-literal=NATS_CLUSTER="$NATS_CLUSTER" --from-literal=CODES_LOAD_CRON="0 22 * * * *" --from-literal=CRON_SCHEDULED_PROCESS_EVENTS_STAN="0 0/1 * * * *" --from-literal=CRON_SCHEDULED_PROCESS_EVENTS_STAN_LOCK_AT_LEAST_FOR="PT55S" --from-literal=CRON_SCHEDULED_PROCESS_EVENTS_STAN_LOCK_AT_MOST_FOR="PT4M" --from-literal=TOKEN_ISSUER_URL="https://$SOAM_KC/auth/realms/$SOAM_KC_REALM_ID" --from-literal=CLIENT_ID="pen-replication-api-service" --from-literal=CLIENT_SECRET="$PR_APIServiceClientSecret" --from-literal=NATS_MAX_RECONNECT=60 --from-literal=HIBERNATE_SQL_LOG_LEVEL=INFO --from-literal=HIBERNATE_PARAM_LOG_LEVEL=INFO --from-literal=DB_CONNECTION_MAX_POOL_SIZE=10 --from-literal=DB_CONNECTION_MIN_IDLE=2 --from-literal=REDIS_URL="redis.$PEN_NAMESPACE-$envValue.svc.cluster.local:6379" --from-literal=CRON_SCHEDULED_PROCESS_RECORDS_TRANSACTION="100" --from-literal=CRON_SCHEDULED_PROCESS_RECORDS_TRANSACTION_LOCK_AT_LEAST_FOR="50ms" --from-literal=CRON_SCHEDULED_PROCESS_RECORDS_TRANSACTION_LOCK_AT_MOST_FOR="60ms" --from-literal=CRON_SCHEDULED_PROCESS_UNCOMPLETED_SAGA="0 0/5 * * * *" --from-literal=CRON_SCHEDULED_PROCESS_UNCOMPLETED_SAGA_LOCKATLEASTFOR="PT4M" --from-literal=CRON_SCHEDULED_PROCESS_UNCOMPLETED_SAGA_LOCKATMOSTFOR="PT4M55S" --from-literal=CRON_SCHEDULED_PROCESS_SCHOOL_DATES="0 0/5 * * * *" --from-literal=CRON_SCHEDULED_PROCESS_EVENTS_SCHOOL_DATES_LOCK_AT_LEAST_FOR="PT4M" --from-literal=CRON_SCHEDULED_PROCESS_EVENTS_SCHOOL_DATES_LOCK_AT_MOST_FOR="PT4M" --from-literal=CRON_SCHEDULED_PROCESS_AUTHORITY_DATES="0 0/5 * * * *" --from-literal=CRON_SCHEDULED_PROCESS_EVENTS_AUTHORITY_DATES_LOCK_AT_LEAST_FOR="PT4M" --from-literal=CRON_SCHEDULED_PROCESS_EVENTS_AUTHORITY_DATES_LOCK_AT_MOST_FOR="PT4M" --from-literal=SPRING_JPA_SHOW_SQL=false --from-literal=MAX_PARALLEL_SAGA=10 --from-literal=PURGE_RECORDS_SAGA_AFTER_DAYS=365 --from-literal=SCHEDULED_JOBS_PURGE_OLD_SAGA_RECORDS_CRON="@midnight" --dry-run -o yaml | oc apply -f -
